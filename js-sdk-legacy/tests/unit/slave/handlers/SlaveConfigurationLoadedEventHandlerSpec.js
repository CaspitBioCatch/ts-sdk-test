...
import SlaveConfigurationLoadedEventHandler from '../../../../src/slave/handlers/SlaveConfigurationLoadedEventHandler';
...
describe('SlaveConfigurationLoadedEventHandler tests:', function () {
    ...
    beforeEach(function () {
        ...
        this.pauseResumeMgrStub = sinon.createStubInstance(PauseResumeManager);
        ...
        this.slaveConfigurationLoadedEventHandler = new SlaveConfigurationLoadedEventHandler(this.messageBus, this.featureServiceStub, this.dataQStub,
            this.pauseResumeMgrStub, this.loggerStub, this.contextMgrStub, this._sensorGateKeeper);
    });
    ...
    it('should handle multiple config updates once multiple events are triggered', function () {
        ...
        assert.isTrue(this.featureServiceStub.updateRunByConfig.calledThrice, 'FeatureService was not called once');
        assert.equal(this.featureServiceStub.updateRunByConfig.firstCall.args[0], this.configurationRepositoryStub, 'FeatureService.updateRunByConfig was not called with expected args');
        ...
        assert.isTrue(this.dataQStub.updateWithConfig.calledThrice, 'DataQ was not called once');
        assert.equal(this.dataQStub.updateWithConfig.firstCall.args[0], this.configurationRepositoryStub, 'DataQ.updateWithConfig was not called with expected args');
        ...
        assert.isTrue(this.pauseResumeMgrStub.onConfigUpdate.calledThrice, 'PauseResumeMgr was not called once');
        assert.equal(this.pauseResumeMgrStub.onConfigUpdate.firstCall.args[0], this.configurationRepositoryStub, 'PauseResumeMgr.onConfigUpdate was not called with expected args');
        ...
        assert.isTrue(this.loggerStub.updateLogConfig.calledThrice, 'Logger was not called once');
        assert.equal(this.loggerStub.updateLogConfig.firstCall.args[0], this.configurationRepositoryStub, 'Logger.updateLogConfig was not called with expected args');
        ...
        assert.isTrue(this.contextMgrStub.onConfigUpdate.calledThrice, 'ContextMgr was not called once');
        assert.equal(this.contextMgrStub.onConfigUpdate.firstCall.args[0], this.configurationRepositoryStub, 'ContextMgr.onConfigUpdate was not called with expected args');
