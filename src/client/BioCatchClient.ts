...
export class BioCatchClient {
	...
	constructor(
		...
		if (this.isDependenciesObject(clientOrDependencies)) {
			...
			this.dynamicCdApiLoader =
				clientOrDependencies.dynamicCdApiLoader;
			this.configMapper = clientOrDependencies.configMapper;
			this.serverUrlResolver =
				clientOrDependencies.serverUrlResolver;
			this.remoteConfigurationLoadedCallback =
				clientOrDependencies
					.remoteConfigurationLoadedCallback;
		} else {
			if (
				!dynamicCdApiLoader || !configMapper ||
				!serverUrlResolver
			) {
				throw new Error(
					"All dependencies must be provided when using individual parameters",
				);
			}
		...
		console.log("initializeBioCatchClient");
		this.initialize();
	}

	private isDependenciesObject(
		obj: any,
	): obj is BioCatchClientDependencies {
		return (
			obj &&
			typeof obj === "object" &&
			"client" in obj &&
			"dynamicCdApiLoader" in obj
		);
	}
	...
	private initialize(): void {
		...
		if (typeof window === "undefined") {
			console.warn(
				"BioCatch SDK: Not in browser environment, skipping initialization",
			);
			return;
		}
	...
	private applyPolyfills(): void {
		...
		try {
			...
			if (typeof window !== "undefined") {
				...
				console.debug(
					"BioCatch SDK: Polyfills applied",
				);
			}
		} catch (error) {
			console.warn(
				"BioCatch SDK: Failed to apply polyfills",
				error,
			);
		}
	}
	...
	private createProxyInterface(): IBioCatchClientProxy {
		return {
			start: (
				...
				const resolvedServerUrl = this.serverUrlResolver
					.resolve(
						startupServerUrl,
						customerID,
						protocolType,
				...
				const startupConfigurations = this.configMapper
				...
				this.dynamicCdApiLoader.attachCdApi(
					window,
					customerSessionID,
				);
